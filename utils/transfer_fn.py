...
class TransferFunction:
    def __init__(self, range_min, range_max, control_vals, opacity=[1.0]*4, colmap='bwr'):
    ...
    def normalize(self, val):
        return (val-self.range_min)/((self.range_max-self.range_min) + EPS)

    def get_color(self, vals):
        rgb = np.zeros((vals.shape[0], 3))
        rgb = self.colmap(self.normalize(vals))
        rgb[(vals < self.range_min) & (vals > self.range_max)] = np.zeros(rgb.shape[1])
        ...
        for i in range(len(self.control_vals)-1):
            ctrl_range = (vals >= self.control_vals[i]) & (vals <= self.control_vals[i+1])
            ratio = (vals - self.control_vals[i]) / ((self.control_vals[i+1] - self.control_vals[i]) + EPS)
            alpha = min(self.opacity[i], self.opacity[i+1]) + ratio * np.abs(self.opacity[i+1] - self.opacity[i])
...
def colorize_particles(data, var, tf):
    ...
    # field = np.linalg.norm(np.squeeze(data.variables[var[0]]), axis=-1)
    # field = field - field.max() / 2.0
    particleCount = field.shape[0]
    print("Getting colors...")
    fcols = tf.get_color(field)

    print("Writing to file...")
    if 'color' not in data.variables.keys():
        data.createDimension("color", 4)
        color = data.createVariable('color' , 'f4', ("T", "P", "color"), zlib=True)
    else:
        color = data.variables['color']
    color[:] = np.expand_dims(fcols, axis=0)
    print("Done")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='NetCDF utilities script')
    parser.add_argument('--fname', type=str, default='particles_008_hd.nc4', help='Name of the netcdf file')
    ...
    range_min = -20.0
    range_max = 20.0
    # control_vals = [-4.0, -4, 4, 4.0]
    # control_vals = [-5.0, -0.75, 0.75, 5.0]
    control_vals = [-7.0, -3.5, 3.5, 7.0]
    # control_vals = [-15.0, -5, 5, 15.0]
    opacity = [1.0, 0, 0, 1.0]
