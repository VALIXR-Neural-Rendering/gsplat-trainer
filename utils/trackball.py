...
# 		self.scale = scale
# 		self.renorm = renorm
# 		self.quat = quat.quaternion(1, 0, 0, 0)
...
# 			self.quat = quat.quaternion(q[0], q[1], q[2], q[3])


# 	def __getattr__(self, name):
# 		if name != 'matrix':
# 			raise AttributeError(f'No attribute named {name}')
# 		return quat.as_rotation_matrix(self.quat)
    ...
    def drag(self, point):
        ...
        if self._state == Trackball.STATE_ROTATE:
            if self.rotation_mode == 'trackball':
                #pdb.set_trace()
                intersection_point = linePlaneCollision(planeNormal=np.array([0,0,1.0]), planePoint=np.array([0,0,0]), rayDirection=direction, rayPoint=eye)

                zax = (intersection_point - eye) / np.linalg.norm(eye - intersection_point)
                ...
                #print("Before:\n", self._n_pose)
                #self._n_pose = x_rot_mat.dot(self._pose)
                ...
                #y_angle = dy / mindim
                #y_rot_mat_ = transformations.rotation_matrix(
                #    y_angle, y_axis, intersection_point
                #)

                ##x_angle = -dx / mindim
                ##x_rot_mat_ = transformations.rotation_matrix(
                ##    x_angle, x_axis, intersection_point
                ##)

                #self._n_pose = y_rot_mat_.dot(self._pose)
                ##self._n_pose = x_rot_mat_.dot(y_rot_mat_.dot(self._pose))
        ...
        # Interpret drag as a zoom motion
            ...
            #translation = -np.sign(dy) * ratio/1000 * radius * z_axis
            #t_tf = np.eye(4)
            #t_tf[:3,3] = translation
            #self._n_pose = t_tf.dot(self._pose)
